...
def reg():
    if request.method == "POST":
        ...
        user = User(email=email, password=password); db.session.add(user); db.session.commit(); session["user_id"] = user.id; return redirect("/")
...
def log():
    if request.method == "POST":
        ...
        if user and user.password == request.form["password"].strip(): session["user_id"] = user.id; return redirect("/")
...
def webhook():
    if not FLW_SECRET_HASH or request.headers.get("verif-hash") != FLW_SECRET_HASH: return jsonify({"status": "invalid"}), 401
    payload = (request.json or {}).get("data", request.json or {})
    if payload.get("status") == "successful":
        email = (payload.get("customer", {}).get("email") or "").lower().strip(); user = User.query.filter_by(email=email).first()
